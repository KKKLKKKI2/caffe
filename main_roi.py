...
def click4():
    ...
    #label8.pack(side="bottom", fill="both", expand="yes")
    #label8.configure(text=str(dir[0]))
    global image_path
    image_path = dir
def click5():
    #txt_dir = openfiledir()
...
def click6():
        ...
        for i in range(1):
            ...
            #cv2.waitKey(5000)
            image='C:/Users\D300_ADAS\Desktop\caffe-windows\python/'+'456.jpg'
            text = txt_data[0].split(' ')
           ...
           # result_1, result_2, result_3, result_4, result_5 = tt.classification(net_file, caffemodel_file, label_file,
                                                                   #image)
            prob, top_k, labels = prediction.startprediction(image, net_file, caffemodel_file, label_file)
            for i in range(1, 6):
                ...
                #print(result_1,result_2,result_3,result_4,result_5)
                    ...
                    #accuracy_top1 += 1
            #elif ((result_2) == (label) or (result_3) == (label) or (result_4) == (label) or (result_5) == (label)):
                    #accuracy_top5 += 1
...
label7=tk.Label(win,text='image file :')
